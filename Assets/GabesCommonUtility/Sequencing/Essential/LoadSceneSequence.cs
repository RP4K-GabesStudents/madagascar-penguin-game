...
namespace Sequencing.Essential
{
    [ExecuteInEditMode]
    public class LoadSceneSequence : MonoBehaviour, IEntrySequence
    {
        ...
        // Whole-operation flag: when false, the server drives this load/unload for all
        // clients; when true, every peer loads/unloads locally. Applies to every scene
        // in this step, not per-scene. Always serialized so it shows in the inspector
        // regardless of whether netcode is installed; only read on the networked paths.
        #if UNITY_NETCODE_GAMEOBJECTS
...
#endif
            Debug.Log($"LoadSceneSequence Initiating. {displayMessage}");

            // Surface the status to any listening loading UI (SequenceEntryPoint
            // re-broadcasts this via OnMessage). The event existed before but was
            // never raised; remove this line if that was intentional.
            DisplayMessage?.Invoke(displayMessage);
        ...
        private async UniTask UnloadAdditive()
        {
            ...
            foreach (var scene in scenesToUnload)
            {
                ...
                {
                    Debug.LogWarning($"There is a null scene reference on the sequence {gameObject.name}", gameObject);
                    continue;
                }
